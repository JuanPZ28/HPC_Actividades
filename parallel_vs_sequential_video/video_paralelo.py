...
WORKERS= 16  #Cantidad de workers a trabajar 
...
def frames_color():
    ...
    #---------------Directorio con frames en color
    while True:
...
#--------------------------#AQUI FINALIZAN LOS WORKERS EN PARALELO

def procesar_video(saved_count,WORKERS):#Asignar saved_count y workers iniciales
    inicio_tiempo=time.perf_counter()
    Subtrabajos= saved_count// WORKERS
    procesos = []
    ...
    if tiempo_ejecucion > 100:
        print(f"Tiempo de ejecucion: {tiempo_ejecucion/60} minuto/s")
    else:
        print(f"Tiempo de ejecucion: {tiempo_ejecucion} segundos")


def video_gris():
    #-------------------------------------------------------------Proceso de frames a video
...
if __name__ == "__main__":
    ...
    saved_count=frames_color()
    procesar_video(saved_count,WORKERS)
    video_gris()
